...
public class AsyncApiChannel {
    ...
    public Map<String, ParameterObject> getParameterObjects() {
        ...
        if (parameterObjects == null) {
            ...
            JsonObject parameters = asyncApiChannel.getAsJsonObject("parameters");
            for (String p : this.getParameters()) {
                JsonObject schemaElt = parameters.getAsJsonObject(p).getAsJsonObject("schema");
                if (schemaElt.has("enum")) {
                    JsonArray enumList = schemaElt.getAsJsonArray("enum");
                    if (enumList.size() > 0) {
                        ParameterObject po = new ParameterObject();
                        for (JsonElement enumElt : enumList) {
                            po.getEnumValues().add(enumElt.getAsString());
                        }
                        this.parameterObjects.put(p, po);
                    // } else {
                    //     this.parameterObjects.put(p, null);
                    }
                }
            }
        }
    ...
    public List<AsyncApiMessage> getOpMessages( String operationType ) throws Exception {
        ...
        // Not sure we can ever get here with a proper AsyncApi 
        return Collections.singletonList( new AsyncApiMessage(message, asyncApi, "DefaultMessageName") );
    }
